...
// Same data as exportUsedLogJSON(), wrapped as a plain <script> global
// (EMBEDDED_USED_QUESTIONS) instead of bare JSON — see
// seedUsageLogFromDataFolder() above. Mirrors tools/rebuild.js's
// buildEmbeddedJS().
function buildEmbeddedUsedQuestionsJS(log) {
    ...
    return (
        ...
        "// ============================================================\n\n" +
        "const EMBEDDED_USED_QUESTIONS = " + JSON.stringify(log, null, 4) + ";\n"
...
async function seedUsageLogFromDataFolder() {
    ...
    // data/used-questions-embedded.js (loaded before this file — see
    // index.html) defines EMBEDDED_USED_QUESTIONS as a plain <script>
    // global when present. That's not blocked under file://, unlike
    // fetch() below, so this is what actually fires for the offline
    // pendrive copy — mirrors data/data-embedded.js's EMBEDDED_APP_DATA
    // pattern used by loader.js.
    if (typeof EMBEDDED_USED_QUESTIONS !== "undefined") {

        importUsedLogFromObject(EMBEDDED_USED_QUESTIONS);
